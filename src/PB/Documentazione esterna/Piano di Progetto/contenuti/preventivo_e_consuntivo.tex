...
\subsubsubsection{Consuntivo settimo periodo: 06/03/2025 - 19/03/2025}
Tutti gli obiettivi predisposti all'inizio del periodo sono stati soddisfatti con successo.\\
...
\paragraph{Prospetto economico settimo periodo: 06/03/2025 - 19/03/2025: }
...
\subsubsubsection{Preventivo ottavo periodo: 19/03/2025 - 03/04/2025}
\begin{table}[h!]
    \centering
    \renewcommand{\arraystretch}{1.5}
    \begin{tabularx}{\textwidth}{|c|X|X|X|X|X|X|c|}\hline
...
\begin{itemize}
    \item Aggiornamento e completamento della documentazione e comunicazione al professor Cardin:
    \begin{itemize}
        \item Aggiornamento del cruscotto di valutazione della qualità nel \emph{Piano di Qualifica};
        \item Inserimento delle immagini definitive dei diagrammi nella \emph{Specifica Tecnica};
        \item Rilettura e riverifica di tutti i documenti per correggere errori e migliorare la coerenza;
        \item Sistemazione del README del \emph{repository} dell’\emph{MVP}, sotto richiesta del proponente;
...
Il prospetto economico relativo al settimo periodo evidenzia i costi sostenuti per ciascun membro del team, suddivisi per ruolo, e il saldo complessivo a fine periodo.\\
L'analisi dei costi si è concentrata sulle ore di lavoro effettivamente registrate.\\
In dettaglio, il costo orario per ogni membro del team è stato calcolato in base al ruolo interpretato, con i costi totali che sono stati ottenuti considerando le ore di lavoro svolte.\\
Il totale delle spese sostenute per il settimo periodo ammonta a 1125 \euro.\\
\begin{table}[!h]
    \centering
    \renewcommand{\arraystretch}{1.5}
    \begin{tabularx}{\textwidth}{|c|X|X|X|X|X|X|c|}\hline
...
Il rischio principale è stato riscontrato solo al termine dei lavori, si tratta dell'assenza del professor Cardin nell'ultima settimana di marzo, che ha rallentato il termine del progetto. Fortunatamente, essendo questa problematica emersa alla fine del progetto, non ha causato ritardi su attività precedenti alla revisione della Product Baseline, tuttavia potrebbe causare ritardi qualora emergano errori a valle della revisione. Inoltre, questa situazione ha costretto un componente del gruppo a posticipare l'inizio del proprio tirocinio, dimostrando l'impatto che i rischi inattesi possono avere sull'efficacia delle attività. Nonostante ciò, il team ha saputo reagire in modo efficace, riuscendo a completare la documentazione con maggiore calma e a terminare le attività di progetto senza incidere significativamente sul costo orario.