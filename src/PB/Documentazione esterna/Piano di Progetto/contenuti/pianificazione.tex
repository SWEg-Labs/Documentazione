...
\item \textbf{\emph{Piano di Qualifica}}: ampliamento della sezione cruscotto di valutazione all’interno del documento seguendo le il feedback ricevuto dalla revisione \emph{RTB}, aggiunta dei test di integrazione e di unità;
...
\end{table}
Il quinto periodo che va dal 6 al 20 febbraio 2025, abbiamo lavorato al salvataggio e recupero dello storico di sessione, migliorato la proposta di domande per guidare la conversazione e ottimizzato l’aggiornamento automatico, introducendo la visualizzazione dell’esito e una gestione più efficiente delle modifiche ai file, specialmente su \emph{GitHub} (sebbene con qualche ritardo dovuto al secondo appello di Ingegneria del Software).
Abbiamo avviato la scrittura della \emph{Specifica Tecnica} e del \emph{Manuale Utente}, focalizzandoci sulla sezione "come fare le domande". Inoltre, aggiorniamo i \emph{casi d’uso} man mano che emergono nuove funzionalità.
Dal punto di vista architetturale, stiamo adottando l’{\emph{architettura esagonale}}\textsubscript{\textit{\textbf{G}}} per garantire maggiore modularità e manutenibilità del sistema. 
